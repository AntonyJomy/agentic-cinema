"""
tests/test_literary_reference_specialist.py

Minimal local end-to-end test for the Quote/Literary Reference Research Specialist.

Uses a real literary quote Entity and performs a REAL Parallel MCP search.
...
APP_NAME = "literary_reference_specialist_test"
USER_ID = "test_user"
TEST_QUOTE = "To be, or not to be, that is the question"
...
def build_test_quote_entity() -> Entity:
    return Entity(
        name=TEST_QUOTE,
        entity_type=EntityType.QUOTE_OR_LITERARY_REFERENCE,
        context=(
            "INT. THEATER - NIGHT. The actor steps into the spotlight and "
            f'recites: "{TEST_QUOTE}."'
        ),
        location=ScriptLocation(
            page_number=9,
            scene_number=6,
            line_excerpt=f'"{TEST_QUOTE}."',
        ),
        confidence=0.96,
    )
...
async def run_literary_reference_specialist(entity: Entity) -> dict:
    runner = InMemoryRunner(app_name=APP_NAME, agent=literary_reference_specialist)
    ...
    prompt = (
        ...
        "entity_type must be treated as quote_or_literary_reference.\n\n"
    ...
    async for event in runner.run_async(
        ...
        for part in event.content.parts:
            ...
            if text and event.author == "literary_reference_research_agent":
    ...
    return {
        ...
        "recheck_feedback_state": recheck_feedback,
    }


def main() -> int:
    require_api_keys()
    entity = build_test_quote_entity()
    outcome = asyncio.run(run_literary_reference_specialist(entity))
    ...
    recheck_ran = (
        ...
        or "literary_reference_confidence_recheck_agent" in outcome["agents_seen"]
        or outcome["recheck_feedback_state"] is not None
    )
    print(f"confidence re-check ran:     {'YES' if recheck_ran else 'NO'}")

    ok = True
    if result.entity_name != TEST_QUOTE:
        print(f"FAILED: entity_name should be {TEST_QUOTE!r}")
        ok = False
    if result.entity_type != EntityType.QUOTE_OR_LITERARY_REFERENCE:
        print("FAILED: entity_type should be quote_or_literary_reference")
        ok = False
