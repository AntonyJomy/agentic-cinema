"""
tests/test_music_specialist.py

Minimal local end-to-end test for the Music Research Specialist.

Uses a real song Entity (Love Story) and performs a REAL Parallel MCP
...
# Mirror music_specialist key bridging for the runner process.
...
APP_NAME = "music_specialist_test"
USER_ID = "test_user"
...
def build_love_story_entity() -> Entity:
    return Entity(
        name="Love Story",
...
async def run_music_specialist(entity: Entity) -> dict:
    runner = InMemoryRunner(app_name=APP_NAME, agent=music_specialist)
    ...
    prompt = (
        ...
        "entity_type must be treated as song.\n\n"
    ...
    async for event in runner.run_async(
        ...
        for part in event.content.parts:
            ...
            if text and event.author == "music_research_agent":
...
def main() -> int:
    require_api_keys()
    entity = build_love_story_entity()
    outcome = asyncio.run(run_music_specialist(entity))
    ...
    recheck_ran = (
        ...
        or "music_confidence_recheck_agent" in outcome["agents_seen"]
        or outcome["recheck_feedback"] is not None
    )
    print(f"confidence re-check ran:     {'YES' if recheck_ran else 'NO'}")

    ok = True
    if result.entity_name != "Love Story":
        print("FAILED: entity_name should be Love Story (from Entity)")
        ok = False
    if result.entity_type != EntityType.SONG:
        print("FAILED: entity_type should be song")
        ok = False
