"""
tests/test_trademark_brand_specialist.py

Minimal local end-to-end test for the Trademark/Brand Research Specialist.

Uses a real logo_brand Entity (Coca-Cola) and performs a REAL Parallel MCP
search (no mocks). Verifies:
  - research finding
  - research confidence
...
APP_NAME = "trademark_brand_specialist_test"
USER_ID = "test_user"
...
def build_coca_cola_entity() -> Entity:
    return Entity(
        name="Coca-Cola",
        entity_type=EntityType.LOGO_BRAND,
        context=(
            "A red delivery truck drives past with the Coca-Cola logo "
            "clearly visible on the side."
        ),
        location=ScriptLocation(
            page_number=3,
            scene_number=2,
            line_excerpt="the Coca-Cola logo clearly visible on the side.",
        ),
        confidence=0.97,
    )
...
async def run_trademark_brand_specialist(entity: Entity) -> dict:
    runner = InMemoryRunner(app_name=APP_NAME, agent=trademark_brand_specialist)
    ...
    prompt = (
        ...
        "entity_type must be treated as logo_brand.\n\n"
    ...
    async for event in runner.run_async(
        ...
        for part in event.content.parts:
            ...
            if text and event.author == "trademark_brand_research_agent":
...
def main() -> int:
    require_api_keys()
    entity = build_coca_cola_entity()
    outcome = asyncio.run(run_trademark_brand_specialist(entity))
    ...
    recheck_ran = (
        ...
        or "trademark_brand_confidence_recheck_agent" in outcome["agents_seen"]
        or outcome["recheck_feedback"] is not None
    )
    print(f"confidence re-check ran:     {'YES' if recheck_ran else 'NO'}")

    ok = True
    if result.entity_name != "Coca-Cola":
        print("FAILED: entity_name should be Coca-Cola (from Entity)")
        ok = False
    if result.entity_type != EntityType.LOGO_BRAND:
        print("FAILED: entity_type should be logo_brand")
        ok = False
