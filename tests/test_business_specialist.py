"""
tests/test_business_specialist.py

Minimal local end-to-end test for the Business Research Specialist.

Uses a real business Entity (McDonald's) and performs a REAL Parallel MCP
...
# Mirror business_specialist key bridging for the runner process.
...
APP_NAME = "business_specialist_test"
USER_ID = "test_user"
...
def build_mcdonalds_entity() -> Entity:
    return Entity(
        name="McDonald's",
        entity_type=EntityType.BUSINESS,
        context=(
            "John walks into McDonald's and orders a coffee. "
            "The golden arches are visible behind him."
        ),
...
async def run_business_specialist(entity: Entity) -> dict:
    runner = InMemoryRunner(app_name=APP_NAME, agent=business_specialist)
    ...
    async for event in runner.run_async(
        ...
        for part in event.content.parts:
            ...
            if text and event.author == "business_research_agent":
...
def main() -> int:
    require_api_keys()
    entity = build_mcdonalds_entity()
    outcome = asyncio.run(run_business_specialist(entity))
    ...
    recheck_ran = (
        ...
        or "confidence_recheck_agent" in outcome["agents_seen"]
        or outcome["recheck_feedback"] is not None
    )
    print(f"confidence re-check ran:     {'YES' if recheck_ran else 'NO'}")

    ok = True
    if result.entity_name != "McDonald's":
        print("FAILED: entity_name should be McDonald's (from Entity)")
        ok = False
    if result.entity_type != EntityType.BUSINESS:
        print("FAILED: entity_type should be business")
        ok = False
