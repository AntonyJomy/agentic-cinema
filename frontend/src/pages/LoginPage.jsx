import { useState } from 'react';
import { Link, Navigate, useLocation, useNavigate } from 'react-router-dom';
import { useAuth } from '../auth/AuthContext';
import './LoginPage.css';

export default function LoginPage() {
  const { configured, isAuthenticated, loading, signInWithGoogle } = useAuth();
  const [error, setError] = useState('');
  const [busy, setBusy] = useState(false);
  const navigate = useNavigate();
  const location = useLocation();
  const redirectTo = location.state?.from || '/upload';

  if (configured && !loading && isAuthenticated) {
    return <Navigate to={redirectTo} replace />;
  }

  async function handleGoogleSignIn() {
    if (!configured) {
      setError(
        'Firebase is not configured yet. Add VITE_FIREBASE_* values to frontend/.env and restart Vite.'
      );
      return;
    }
    setError('');
    setBusy(true);
    try {
      await signInWithGoogle();
      navigate(redirectTo, { replace: true });
    } catch (err) {
      const message =
        err?.code === 'auth/popup-closed-by-user'
          ? 'Sign-in was cancelled.'
          : err?.message || 'Google sign-in failed.';
      setError(message);
    } finally {
      setBusy(false);
    }
  }

  return (
    <div className="login-page">
      <div className="login-panel">
        <Link to="/" className="login-brand">
          ScriptClear <span>AI</span>
        </Link>

        <p className="login-eyebrow">Restricted access</p>

        <h1 className="login-title">Sign in to continue</h1>
        <p className="login-copy">
          Use your Google account to run clearance and record legal decisions.
        </p>

        <button
          type="button"
          className="login-google-btn"
          onClick={handleGoogleSignIn}
          disabled={busy || loading}
        >
          <GoogleMark />
          <span>{busy ? 'Signing in…' : 'Continue with Google'}</span>
        </button>

        {error ? <p className="login-error">{error}</p> : null}

        {!configured ? (
          <p className="login-setup-hint">
            Local preview — add <code>VITE_FIREBASE_*</code> to{' '}
            <code>frontend/.env</code> to enable Google sign-in.
          </p>
        ) : null}
      </div>
    </div>
  );
}

function GoogleMark() {
  return (
    <svg width="18" height="18" viewBox="0 0 48 48" aria-hidden="true">
      <path
        fill="#FFC107"
        d="M43.6 20.5H42V20H24v8h11.3C33.7 32.7 29.3 36 24 36c-6.6 0-12-5.4-12-12s5.4-12 12-12c3.1 0 5.8 1.2 8 3.1l5.7-5.7C34.2 6.1 29.4 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20 20-8.9 20-20c0-1.3-.1-2.5-.4-3.5z"
      />
      <path
        fill="#FF3D00"
        d="M6.3 14.7l6.6 4.8C14.7 16 19 12 24 12c3.1 0 5.8 1.2 8 3.1l5.7-5.7C34.2 6.1 29.4 4 24 4 16.3 4 9.6 8.3 6.3 14.7z"
      />
      <path
        fill="#4CAF50"
        d="M24 44c5.2 0 9.9-2 13.4-5.2l-6.2-5.2C29.3 35.9 26.8 37 24 37c-5.2 0-9.6-3.3-11.3-7.9l-6.5 5C9.5 39.6 16.2 44 24 44z"
      />
      <path
        fill="#1976D2"
        d="M43.6 20.5H42V20H24v8h11.3c-1.1 3.1-3.5 5.5-6.5 6.6l.1.1 6.2 5.2C36.8 41.2 44 36 44 24c0-1.3-.1-2.5-.4-3.5z"
      />
    </svg>
  );
}
